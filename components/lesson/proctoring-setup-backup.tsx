...
export default function ProctoringSetup({ onComplete, onCancel }: ProctoringSetupProps) {
    ...
    useEffect(() => {
        ...
        // CRITICAL FIX 4: Create downscaled canvas for stable detection
        ...
        const detectFace = async (time: number) => {
            ...
            // CRITICAL FIX 2: Throttle to ~12 FPS (80ms) to prevent overload
            if (time - lastTime < 80) {
                animationId = requestAnimationFrame(detectFace)
                return
            }
            ...
            try {
                // CRITICAL FIX 4: Downscale input for stable detection
                smallCtx.drawImage(video, 0, 0, 256, 256)

                // Run face detection on downscaled image
                const predictions = await modelRef.current.estimateFaces(smallCanvas, false)
                const numFaces = predictions.length
                setFaceCount(numFaces)

                // Scale factor for coordinates
                const scaleX = video.videoWidth / 256
                const scaleY = video.videoHeight / 256
                ...
                if (numFaces === 0) {
                    ...
                    const faceWidth = (end[0] - start[0]) * scaleX
                    const faceHeight = (end[1] - start[1]) * scaleY
                    ...
                    if (faceArea < 0.05) {
                        ...
                        const faceCenterX = (start[0] * scaleX) + faceWidth / 2
                        const faceCenterY = (start[1] * scaleY) + faceHeight / 2
                        ...
                        if (offsetX > 0.25 || offsetY > 0.25) {
                            ...
                            if (ctx) {
                                ...
                                const faceImageData = ctx.getImageData(
                                    Math.max(0, start[0] * scaleX),
                                    Math.max(0, start[1] * scaleY),
    ...
    const handleContinue = () => {
        if (step === "intro") {
            ...
            // Require face detection and good lighting
            if (faceDetected && lightingGood && modelLoaded) {
                setStep("ready")
            }
        } else if (step === "ready") {
            if (stream) {
